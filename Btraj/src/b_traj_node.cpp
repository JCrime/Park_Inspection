...
#include "sensor_handler/yellow_lane.h"
...
// ros related
ros::Subscriber _map_sub, _pts_sub, _odom_sub,_lane_sub;
ros::Publisher _fm_path_vis_pub, _local_map_vis_pub, _inf_map_vis_pub, _corridor_vis_pub, _traj_vis_pub, _grid_path_vis_pub, _nodes_vis_pub, _traj_pub, _checkTraj_vis_pub, _stopTraj_vis_pub,_error_pub;
...
sensor_handler::yellow_lane _lane;
...
void rcvLaneCallback(const sensor_handler::yellow_lane lane);
...
{   
    ...
    if(_is_use_fm)
    {
            ...
            for(unsigned int j = 0; j < size_y; j++)
            {
                for(unsigned int i = 0; i < size_x; i++)
                {
                    ...
                    if(collision_map_local->Inside(index))
                    {
                        ...
                        if(_lane.isValid)
                        {
                          if(_lane.vec_x*(pt[1]-_lane.p_y)-_lane.vec_y*(pt[0]-_lane.p_x)>0)
                          {
                            flow_vel *= 0.2;
                          }
                        }
        ...
        if(fm_solver->compute(max_vel) == -1)
        {
            ...
            std_msgs::String err2;
            err2.data = "goal_error";
            _error_pub.publish(err2);
         ...
         _cube_margin, _is_limit_vel, _is_limit_acc, obj, _bezier_coeff ) == -1 )
    {
        ...
        std_msgs::String err1;
        err1.data = "solver_error";
        _error_pub.publish(err1);
    }
    else
    {   
...
int main(int argc, char** argv)
{
    ...
    _map_sub  = nh.subscribe( "/points_no_ground",       1, rcvPointCloudCallBack );
    _odom_sub = nh.subscribe( "/car_odo",  1, rcvOdometryCallbck);
    _pts_sub  = nh.subscribe( "/waypoints", 1, rcvWaypointsCallback );
    _lane_sub = nh.subscribe("/yellow_lane",1,rcvLaneCallback);
    ...
    _error_pub         = nh.advertise<std_msgs::String>("/error_msg",1);
